...
- MySQL / MariaDB
- Apache with `mod_rewrite` enabled (e.g. via [XAMPP](https://www.apachefriends.org/) or [Laragon](https://laragon.org/))
...
1. Clone the repository into your local server's document root:
   ...
   ```
2. Create a MySQL database (e.g. `petHero`) and import the schema:
   ```bash
   mysql -u root -p petHero < PetHero/Config/DB/PetHero-DB.sql
   ```
3. Copy the example environment file and set your own local credentials:
   ```bash
...
4. Point your Apache virtual host / document root to the `PetHero/` folder and make sure `.htaccess` rewrite rules are enabled.
5. Open the app in your browser and you should see the Pet Hero home page.

## Author

Built by [Ignacio Ríos](https://github.com/nachoq22) with Misael Flores as part of the UTN Software Engineering program.